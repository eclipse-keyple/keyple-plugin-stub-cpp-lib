...
TEST(StubPoolPluginFactoryAdapterTest, init_factory_with_reader_configuration)
{
    ...
    auto reader
        = std::dynamic_pointer_cast<StubReaderAdapter>(stubPlugin->searchReader(READER_NAME));
    ...
    auto reader2
        = std::dynamic_pointer_cast<StubReaderAdapter>(stubPlugin->searchReader(READER_NAME_2));
