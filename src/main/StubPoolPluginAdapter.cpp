    ...
    const int monitoringCycleDuration)
{
    ...
    mStubPluginAdapter
        = std::make_shared<StubPluginAdapter>(name, configurations, monitoringCycleDuration);

    for (const auto& readerConfiguration : readerConfigurations) {
        mReaderToGroup.insert(
            {readerConfiguration->getName(), readerConfiguration->getGroupReference()});
    }
}
...
StubPoolPluginAdapter::releaseReader(std::shared_ptr<ReaderSpi> readerSpi)
{
    ...
    const auto it
        = std::find(mAllocatedReaders.begin(), mAllocatedReaders.end(), readerSpi->getName());
